...
- `segs_loh`: a dataframe of regions of clonal LOH to exclude from baseline during analysis; either produced by `detect_clonal_loh` or user-provided. Note that `call_clonal_loh` is now recommended over this option. 
...
In samples with high tumor purity (e.g., tumor cell lines) without matched normal cells, heterozygous SNPs are challenging to identify in regions of LoH, leading to decreased power of CNV detection. In these cases, we recommend setting `call_clonal_loh = TRUE` to identify and exclude regions of clonal deletions/LOH. It enables Numbat to run the `detect_clonal_loh` function inside the main workflow.
