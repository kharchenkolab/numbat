...
```
docker run -v /work:/mnt/mydata -it pkharchenkolab/numbat-rbase:main /bin/bash
```
...
*Others*
- `segs_loh`: a dataframe of regions of clonal LOH to exclude from baseline during analysis. See [Detecting clonal LOH](#detecting-clonal-loh) for details.
- `segs_consensus_fix`: a dataframe of pre-determined consensus CNVs (for example, from bulk WGS/WES analysis) that will stay fixed during analysis. See [Using prior segmentation](#using-prior-segmentation) for details.
...
## Using prior segmentation
Sometimes users have access to prior segmentation data (e.g. from bulk WGS/WES analysis). In this case, we recommend using the `segs_consensus_fix` parameter to fix the CNV calls in those regions. To do so, you may provide a dataframe with the following columns:
...
Please note that diploid segments (`cnv_state = "neu"`) should also be included.
