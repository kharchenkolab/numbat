...
From the phylo-heatmap, we can see that clone 5 in particular harbors significantly more CNV events than the rest of the tumor cells. Let's visualize the spatial distribution of subclones (clone_post$clone_opt) and overlap with tissue image.
```
...
**Figure 4. Clonal assignments in BRCA0.** The maximum-a-posteriori clonal assignment of each spot is shown.

Interestingly, if you zoom in close enough, you can see that the distribution of clone 5 corresponds to visible foci that are clearly distinct from the surrounding tumor cells. Taken together, these observations suggest that clone 6 is the most evolved tumor subpopulation that is actively invading the diseased tissue.
