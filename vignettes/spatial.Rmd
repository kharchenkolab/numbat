...
<meta name="twitter:title" content="Allele-specific CNV analysis from spatial transcriptomics" />
<meta name="twitter:description" content="Allele-specific CNV analysis from spatial transcriptomics" />
<meta name="twitter:image" content="https://i.imgur.com/hZpNWVJ.jpg" />
...
We see that there is substantial heterogeneity among CNV events in their spatial distribution. In particular, deletions on chromosomes 9, 14, 16, 17, and 19 seem to mark specific patches of the tissue section.
...
```
nb[['BRCA0']]$plot_phylo_heatmap()
```
![](https://i.imgur.com/sD4DIki.png)
**Figure 4. Tumor phylogeny and CNV heatmap of BRCA0.** 

From the phylo-heatmap, we can see that clone 6 in particular harbors significantly more CNV events than the rest of the tumor cells. Let's visualize the spatial distribution of clone 6 (clone_post$p_6) and overlap with tissue image.
```
...
spots[[sample]] %>%
    ...
    background_image(img) +
    geom_point(aes(color = p_6), size = 0.3, stroke = 0.3, alpha = 0.8, pch = 21) +
...
![](https://i.imgur.com/QT8g6b1.jpg)
**Figure 4. Probabilities of assignment to clone 6.** Opacity of spots corresponds to the probability of the spot belonging to clone 6.

Interestingly, if you zoom in close enough, you can see that the distribution of clone 6 corresponds to visible foci that are clearly distinct from the surrounding tumor cells. Taken together, these observations suggest that clone 6 is the most evolved tumor subpopulation that is actively invading the diseased tissue.
