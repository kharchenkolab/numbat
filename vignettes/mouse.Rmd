...
# Introduction
The mouse is a major model system for studying cancer and many other areas of biology. According to the 10x [website](https://www.10xgenomics.com/resources/publications?refinementList%5BproductGroups%5D%5B0%5D=Single%20Cell%20Gene%20Expression&refinementList%5Btags%5D%5B0%5D=Cancer%20Research&page=1), there are currently 259 cancer single-cell studies on mouse subjects, as compared to 362 on humans. 

Previously, we have shown that with prior haplotype information, Numbat can detect allele-specific CNVs at a much higher accuracy [1]. Although we don't usually know the haplotype of a given human subject *a priori*, it can be guessed using [population-based phasing](https://en.wikipedia.org/wiki/Haplotype_estimation). The mouse is a special case because the haplotypes of most common laboratory strains are fully known (thanks to the mouse genome projects [2-3]). This means that all heterozygous SNPs in a F1 hybrid mouse of two inbred strains are *fully phased* at whole-genome level. In addition, the SNP density in a F1 hybrid mouse can be many times higher than in humans [4]. As we will see, these factors provide a huge advantage for haplotype-aware CNV analysis. 

The most updated version of Numbat (v1.2) now accepts (F1 hybrid) mouse data. The input format is the same (expression counts in a matrix, allele counts in a long dataframe), but some bioinformatics is required to prepare the phased allele counts. This tutorial will walk through data prepration to analyze a mouse PDAC scRNA-seq dataset provided by [Pitter et al](https://doi.org/10.1158/0008-5472.CAN-22-1742) [5].
...
## 2. Run pileup
Now we can generate single-cell allele counts for the SNP positions in the above VCF using `cellsnp-lite`. 
```shell
cellsnp-lite \
...
# Results
Let's look at the results for sample KPT_062521 (SRR20462475), where we see normal diploid cells and two tumor subclones with distinct copy number profiles. 

```r
nb = Numbat$new(glue('./results/{sample}'), i = 2)
```

```r
options(repr.plot.width = 8, repr.plot.height = 4, repr.plot.res = 200)

clone_pal = c('1' = 'gray', setNames(numbat:::pal, as.character(2:9)))

nb$plot_phylo_heatmap(pal_clone = clone_pal, clone_stack = T, p_min = 0.9)
```
![](https://i.imgur.com/VXsuJaP.png)

Let's compare the copy number profiles of the two subclones (clone 4 and 6) in a pseudobulk HMM view.
```
options(repr.plot.width = 10, repr.plot.height = 4.5, repr.plot.res = 200)

nb$bulk_clones %>% filter(sample %in% c(4,6)) %>% 
filter(CHROM %in% c(2, 4, 8, 9)) %>%
plot_bulks(min_depth = 15, use_pos = F, exp_limit = 2.5, min_LLR = 10)
```
![](https://i.imgur.com/2BG5LOY.png)

We see haplotype-specific differences with remarkable resolution. For example, the subclonal copy number profiles for chr2 and chr8 largely agree, whereas chr9 and chr4 have distinct breakpoints and mirorred CNLOH (chr4, middle segment). All alleles collapse to one side in a CNV region, thanks to the perfect phasing. 

# Conclusion

Haplotype-aware CNV analysis is espcially powerful for model systems (mice in particular) where the haplotypes are fully known. Luckily, the Numbat analysis framework is written generically enough so that minimal tweaking is required (simply setting phase switch rate to be zero is sufficient). The main limitation is that Numbat is only applicable to F1 hybrid mice with mixed genetic backgrounds, whereas many mouse tumor models are conducted on a inbred (pure) genetic background, which lacks heterozygous SNPs.
...
1. Keane, T., Goodstadt, L., Danecek, P. et al. Mouse genomic variation and its effect on phenotypes and gene regulation. Nature 477, 289–294 (2011). https://doi.org/10.1038/nature10413

1. Lilue, J., Doran, A.G., Fiddes, I.T. et al. Sixteen diverse laboratory mouse reference genomes define strain-specific haplotypes and novel functional loci. Nat Genet 50, 1574–1583 (2018). https://doi.org/10.1038/s41588-018-0223-8

1. Markowski, J. et al. GAMIBHEAR: whole-genome haplotype reconstruction from Genome Architecture Mapping data, Bioinformatics, Volume 37, Issue 19, 1 October 2021, Pages 3128–3135, https://doi.org/10.1093/bioinformatics/btab238

1. Kenneth L. Pitter, et al. Systematic Comparison of Pancreatic Ductal Adenocarcinoma Models Identifies a Conserved Highly Plastic Basal Cell State. Cancer Res 1 October 2022; 82 (19): 3549–3560. https://doi.org/10.1158/0008-5472.CAN-22-1742